...
| 3b | **Live Agent Console (rmux)** | In the cockpit, open the **Agent Console / rmux** tab for the running task — the actual agent terminal panes stream live; mention remote-control is on by default (the session shows in claude.ai/code) | "And this is the part people don't expect: the live agent terminals. AIFactory runs with **rmux**, so the cockpit streams the real coder/QA panes as they work — and because remote control is on by default, you can even drive this session from claude.ai/code or the mobile app." | 0:40 |
