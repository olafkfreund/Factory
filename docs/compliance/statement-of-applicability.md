...
| A.5.6 | Contact with special interest groups | Implemented | Coordinated disclosure via `SECURITY.md` (PFactory/AIFactory/TFactory); dependency advisories via Renovate/GitHub. | Security Owner |
...
| A.5.21 | Managing information security in the ICT supply chain | Implemented | Dependency pinning (`flake.lock`, digest-pinned bases), Trivy P0 gate, Renovate ([supply-chain-integrity.md](policies/supply-chain-integrity.md)). | Supply-chain |
...
| A.8.8 | Management of technical vulnerabilities | Implemented | CodeQL (5 repos), Trivy P0 gate (`test_p0_supply_chain.py`), Renovate. Partial: no remediation SLA, CFactory unscanned (Factory#317). | Vuln |
